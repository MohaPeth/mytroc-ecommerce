...
const ServiceItem = ({ icon, title, description, isLast = false }: ServiceItemProps) => (
  <div className={`flex flex-col items-center p-6 text-center ${!isLast ? 'border-r border-gray-200' : ''}`}>
    <div className="w-14 h-14 flex items-center justify-center bg-green-500 rounded-full mb-3">
      {icon}
    </div>
    <h3 className="font-bold text-gray-700 uppercase mb-1">{title}</h3>
